...
from tests.errors import DATA_CONVERT_FAIL, OPERATION_FAIL
...
async def test_registration(ac: AsyncClient):
    response = await ac.post("/auth/signin", json={"username": "user_test", "password": "password_test"})
...
async def test_authentication(ac: AsyncClient):
    ...
    assert response.status_code == 200, OPERATION_FAIL
    assert response.cookies.get("access_token") == response.json().get(
        "access_token"
